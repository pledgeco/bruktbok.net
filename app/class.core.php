...
class core implements iCore
{
	...
	final public function handleCall($k)
	{
		...
		if($_CONFIG['bruktbok']['in_maint'] == false)
		{
			if(!isset($_SESSION['user']['id']))
			{
				switch($k)
				{
					...
					case "news":
					case "client":
						header('Location: '.$_CONFIG['bruktbok']['url'].'/index');
						exit;
					break;
			...
			else
			{
				...
				{
					header('Location: '.$_CONFIG['bruktbok']['url'].'/logout');
				}
				...
				switch($k)
				{
					case "index":
					case null:
						header('Location: '.$_CONFIG['bruktbok']['url'].'/me');
					exit;
					break;
		...
		{
			header('Location: '.$_CONFIG['bruktbok']['url'].'/maintenance');
			exit;
		}
	}
	...
	final public function handleCallHK($k)
	{
		...
		if($_SESSION["in_hk"] != true)
		{
			if(isset($_SESSION['user']['id']))
			{
				...
				{
					header("Location:".$_CONFIG['bruktbok']['url']."/ase/login");
					exit;
				}
			}
			else
		...
		else
		{
			if(!isset($k))
			{
				header("Location:".$_CONFIG['bruktbok']['url']."/ase/dash");
				exit;
			}
			else
			{
				if($k == 'balist')
				{
					...
					if(isset($_GET["unban"]))
					{
						...
						header("Location: ".$_CONFIG['bruktbok']['url']."/ase/banlist");
						exit;
					}	
				}
			}
